...
def parse_color(color_str: str):
    ...
    raise ValueError(f"Unknown color '{color_str}'. Use a name (red, blue, ...) or hex (#RRGGBB).")
...
def create_border(page_width, page_height, margin=20, line_width=2, color=(0, 0, 0)):
    ...
    # rectangle inset by 'margin' on all sides
...
def add_borders(input_pdf, output_pdf, margin=20, color=(0,0,0), line_width=2):
    ...
    for page in reader.pages:
        ...
        page.merge_page(overlay)  # draws on top; content remains unchanged
...
def main(argv=None):
    ...
    p.add_argument("--margin", type=float, default=20.0, help="Border inset from page edges in points (default: 20pt ≈ 0.28in)")
    p.add_argument("--color", default="black", help="Border color name or hex (#RRGGBB). Default: black")
    ...
    print(f"Done: '{args.output}' (margin {args.margin} pt, color {args.color}, width {args.width} pt)")
